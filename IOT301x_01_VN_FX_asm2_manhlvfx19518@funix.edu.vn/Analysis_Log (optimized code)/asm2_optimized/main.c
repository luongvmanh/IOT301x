...
int soBanTinGuiDi(char str[])
{
    ...
    int i = 0;
    char searchStr[] = "\"cmd\":\"set\""; //Use strstr() to find \"cmd\":\"set\" save in numSet
    int numSet = 0;
    while (token != 0)
    {
        strcpy(newStr[i], token); //Save each token to newStr array.
        char *p = (strstr(newStr[i], searchStr));
        ...
        ++i;
        token = strtok(0, delimiter); //Take the next substring
    }
    return numSet;
}
...
int soBanTinGuiDenThietBi(char str[])
{
    ...
    token = strtok(strCopy, delimiter);
    int i = 0;
    char searchStr2[] = "\"cmd\":\"set\""; //Use strstr() to find \"cmd\":\"set\"
    while (token != 0)
    {
        strcpy(newStr[i], token); //Save each token to newStr array.
        char *p = strstr(newStr[i], searchStr1);
        char *q = strstr(newStr[i], searchStr2);
